...
export const emailService = {
  ...
  async sendNewOrderToSeller(order, seller) {
    try {
          ...
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="order-info">
                <p><strong>Pedido:</strong> #${order.order_number || 'N/A'}</p>
                ...
                <tbody>
                  ${items.map(item => `
                    <tr>
                      ...
                      <td style="text-align: right;">$${(item.subtotal || item.price || 0).toFixed(2)}</td>
                    </tr>
                  `).join('')}
                </tbody>
              </table>
      ...
      return await this.sendEmail({
        to: seller.email,
        subject: `📦 Nuevo pedido #${order.order_number || 'N/A'}`,
        html
      })
    } catch (error) {
  ...
  async sendNewOrderToClient(order) {
    try {
      ...
      return await this.sendEmail({
        ...
        subject: `✅ Pedido #${order.order_number || 'N/A'} confirmado`,
        html
      })
    } catch (error) {
  ...
  // Enviar notificación de cambio de estado al cliente (para cambios globales)
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              ${notes ? `<div class="notes"><strong>📝 Nota del vendedor:</strong><br>${notes}</div>` : ''}
  ...
  // Enviar notificación de cambio de estado de item AL CLIENTE (para cambios individuales por vendedor)
  async sendItemStatusToClient(order, item, status, notes = '') {
    try {
      const statusMessages = {
        processing: '📦 Un producto de tu pedido está siendo procesado',
        shipped: '🚚 Un producto de tu pedido ha sido enviado',
        delivered: '✅ Un producto de tu pedido ha sido entregado',
        cancelled: '❌ Un producto de tu pedido ha sido cancelado'
      }
      ...
      const statusDetails = {
        processing: {
          ...
          message: 'Uno de los productos de tu pedido está siendo preparado para el envío.'
        },
        shipped: {
          title: 'Producto enviado',
          message: 'Uno de los productos de tu pedido está en camino.'
        },
        delivered: {
          title: 'Producto entregado',
          message: 'Uno de los productos de tu pedido ha sido entregado exitosamente.'
        },
        cancelled: {
          title: 'Producto cancelado',
          message: 'Uno de los productos de tu pedido ha sido cancelado.'
        }
      }

      const detail = statusDetails[status] || { 
        title: 'Actualización de producto', 
        message: 'El estado de un producto de tu pedido ha cambiado.' 
      }
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="item-info">
                <p><strong>Pedido:</strong> #${order.order_number || 'N/A'}</p>
                <p><strong>Producto:</strong> ${item.product_name || 'Producto'}</p>
      ...
      console.error('❌ Error al enviar email de item al cliente:', error)
      return { success: false, error: error.message }
    }
  },
  ...
  try {
    console.log('📧 Iniciando notificación a vendedores...')
    ...
    // Obtener vendedores únicos con sus datos completos
    const sellerMap = {}
    
    items.forEach(item => {
      const sellerId = item.seller_id
      console.log(`🔍 Procesando item: ${item.product_name}, seller_id: ${sellerId}`)
      
      if (sellerId && !sellerMap[sellerId]) {
        // Buscar el perfil del vendedor en los datos del item
        const sellerProfile = item.profiles || null
        
        // Si no hay perfil en el item, intentar obtenerlo de la base de datos
        if (!sellerProfile && item.seller_id) {
          // Aquí podrías hacer una consulta a la BD para obtener el vendedor
          // Por ahora, usamos datos básicos
          sellerMap[sellerId] = {
            id: sellerId,
            email: item.seller_email || `vendedor_${sellerId}@apex.com`,
            name: item.seller_name || `Vendedor ${sellerId.substring(0, 8)}`
          }
        } else if (sellerProfile) {
          sellerMap[sellerId] = {
            id: sellerId,
            email: sellerProfile.email || `vendedor_${sellerId}@apex.com`,
            name: sellerProfile.name || `Vendedor ${sellerId.substring(0, 8)}`
          }
        }
        
        console.log(`👤 Vendedor agregado: ${sellerMap[sellerId].email}`)
      }
    })
    ...
    if (!sellerIds.length) {
      console.warn('⚠️ No se encontraron vendedores para notificar')
      return { success: true, total: 0 }
    }

    console.log(`📧 Notificando a ${sellerIds.length} vendedores:`, sellerIds)

    const results = []

    for (const sellerId of sellerIds) {
      let seller = sellerMap[sellerId]

      // Si el email parece ser un placeholder, intentar obtener el perfil real desde la BD
      if (seller && (!seller.email || seller.email.startsWith('vendedor_') || !seller.email.includes('@'))) {
        try {
          if (isSupabaseConfigured()) {
            ...
            if (!error && profile && profile.email) {
              ...
              console.log(`🔎 Email real encontrado para vendedor ${sellerId}: ${seller.email}`)
            } else {
              console.warn(`⚠️ No se encontró email real para vendedor ${sellerId}, usando fallback: ${seller.email}`)
            }
          } else {
            console.warn('⚠️ Supabase no está configurado; no se puede obtener email real del vendedor')
          }
        } catch (err) {
          console.error('❌ Error al obtener perfil del vendedor desde Supabase:', err)
        }
      }

      if (seller && seller.email) {
        console.log(`📤 Enviando email a vendedor: ${seller.email}`)

        // Filtrar los items que pertenecen a este vendedor
        ...
        const result = await this.sendNewOrderToSeller(orderCopy, seller)
        results.push({ sellerId, ...result })
        if (result.success) {
          console.log(`✅ Email enviado correctamente a ${seller.email}`)
        } else {
          console.error(`❌ Falló el envío a ${seller.email}:`, result.error)
        }
      }
    }

    const allSuccess = results.every(r => r.success)
    console.log(`📊 Resumen: ${results.filter(r => r.success).length}/${results.length} emails enviados correctamente`)
