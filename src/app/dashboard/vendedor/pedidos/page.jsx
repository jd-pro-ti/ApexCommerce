...
const labels = { pending: 'Pendiente', processing: 'En proceso', shipped: 'Enviado' };
const colors = {
...
export default function SellerOrders() {
  ...
  const changeStatus = async (item, status) => {
    if (!confirm(`¿Cambiar "${item.product_name}" a ${labels[status]}?`)) return;
            ...
            <div className="divide-y divide-gray-100">
              {order.items?.map(item => {
                ...
                return <div key={item.id} className="flex flex-wrap items-center gap-4 p-5">
                  ...
                  {status !== 'shipped' && <div className="flex gap-2">
